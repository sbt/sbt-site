...
object RubyHelpers {
  ...
  /** Keys for ruby-based generators.
    ...
    */
  trait RubyKeys {
    val requiredGems = SettingKey[Map[String, String]]("required-gems", "Required gem + versions for this build.")
    val checkGems    = TaskKey[Unit]("check-gems", "Tests whether or not all required gems are available.")
  }
  ...
  def checkGems(requirements: Map[String, String], s: TaskStreams): Unit = {
    ...
    def makeError(gem: String, version: String, current: Option[String]): Option[String] = current match {
      ...
      case Some(v) =>
        Some("This build requires the gem [%s (%s)] but found version (%s) instead." format (gem, version, v))
